...
class EmergencySeverityMatrix(db.Model):
    ...
    level = db.Column(db.String(20), nullable=False, unique=True) # ADVISORY, MINOR, MAJOR, CRITICAL, CATASTROPHIC
...
class EmergencyIncident(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    severity = db.Column(db.String(20), db.ForeignKey('emergency_severity_matrix.level'), nullable=False)
...
class ProtocolDocument(db.Model):
    ...
    category = db.Column(db.String(50), nullable=False) # MEDICAL, FIRE, SECURITY, WEATHER, LOST_CHILD, ACCESSIBILITY, TRANSPORT, CROWD_MANAGEMENT
