...
from models_volunteer import Volunteer
...
def seed():
    with app.app_context():
        ...
        for z, cz in zip(zones, crowd_zones):
            snapshots.append(CrowdSnapshot(zone_id=cz.id, occupancy=z.current_occupancy))
        ...
        db.session.add(
            Recommendation(
                ...
                ai_confidence=0.92,
                proposed_action="Dispatch immediate medical and security teams. Reroute incoming fans to North Gate.",
                approved=False,
            )
        )
        db.session.add(
            Recommendation(
                ...
                ai_confidence=0.75,
                proposed_action="Send maintenance to inspect turnstile 4.",
                approved=False,
            )
        )
        db.session.commit()
        ...
        parking = [
            ParkingZone(
                name="Lot A (VIP)", max_capacity=500, current_occupancy=480, zone_type="VIP"
            ),
            ParkingZone(
                name="Lot B (General)",
                max_capacity=2000,
                current_occupancy=1950,
                zone_type="General",
            ),
            ParkingZone(
                name="Lot C (Accessible)", max_capacity=200, current_occupancy=120, zone_type="ADA"
            ),
            ParkingZone(
                name="Lot D (EV Charging)", max_capacity=100, current_occupancy=90, zone_type="EV"
            ),
        ]
        ...
        decisions = [
            AIArbitrationDecision(
                source_ai_domain="CROWD",
                action_code="REROUTE_GATE_A",
                priority_level="HIGH",
                confidence_score=0.88,
                trigger_event="Congestion > 85%",
                execution_status="EXECUTED",
            ),
            AIArbitrationDecision(
                source_ai_domain="TRANSPORT",
                action_code="DISPATCH_SHUTTLE",
                priority_level="MEDIUM",
                confidence_score=0.91,
                trigger_event="Lot B Full",
