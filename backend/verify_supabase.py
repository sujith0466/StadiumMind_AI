...
def main():
    ...
    print(f"python-dotenv loaded correctly: YES")
    ...
    with app.app_context():
        try:
            ...
            print(f"SSL Status: {'Enabled' if 'ssl' in db_url.lower() or 'supabase' in host else 'Unknown'}")
            
            # STEP 5
            print(f"\n[STEP 5 - SUPABASE VERIFICATION]")
            print(f"Is this Supabase? {'YES' if 'supabase' in host or 'supabase' in version.lower() else 'NO'}")
            ...
            print("10. Frontend Status: Backend API relies on SQLAlchemy models, which are successfully bound to this database. React Axios calls will now natively hit the Live Supabase DB.")
            ...
            if "translate host name" in str(e) or "Name or service not known" in str(e):
                ...
                import subprocess
                try:
                    res = subprocess.check_output(f"Resolve-DnsName {host} -Type AAAA", shell=True).decode()
                    print(f"AAAA Record found:\n{res}")
                    print("Real Cause: Supabase endpoints are IPv6-only. Local system lacks IPv6 routing or native IPv6 DNS resolution.")
                    print("\n[FINAL OUTPUT]")
                    print("1. Active DATABASE_URL: " + mask_url(db_url))
                    print("2. Connection Status: FAIL")
                    print("3. Root Cause: Supabase enforces IPv6 on db.* subdomains. The local network does not support IPv6 routing, resulting in Timeout/DNS failures.")
                    print("4. Fixes Applied: Attempted direct TCP over IPv6, but network lacks IPv6 support.")
                    print("11. Remaining Blockers: Requires an IPv4 connection pooler URL (e.g. pooler.supabase.com) from the Supabase dashboard since local IPv6 is unavailable. Cannot be fixed automatically without the pooler URL/region.")
                except:
