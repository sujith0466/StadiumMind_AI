...
# Import models
from models_emergency import (
    EmergencySeverityMatrix,
    EmergencyIncident,
    FirstResponder,
)
from models import StadiumZone, Incident, OperationEvent, Recommendation
from models_crowd import CrowdZone, CrowdSnapshot
...
def get_historical_timestamps(count=20):
    ...
    for _ in range(count // 4):
        timestamps.append(
            now - timedelta(days=random.randint(2, 7), minutes=random.randint(0, 1440))
        )
    # 30 days ago
    for _ in range(count - len(timestamps)):
        timestamps.append(
            now - timedelta(days=random.randint(8, 30), minutes=random.randint(0, 1440))
        )
    ...
    try:
        ...
        if not db.session.query(EmergencySeverityMatrix.query.exists()).scalar():
            sev_levels = [
                EmergencySeverityMatrix(
                    ...
                    escalation_policy="LOG_ONLY",
                ),
                EmergencySeverityMatrix(
                    level="LOW",
                    response_sla_mins=25,
                    required_roles=["STEWARD"],
                    notification_scope="LOCAL",
                    escalation_policy="LOG_ONLY",
                ),
                EmergencySeverityMatrix(
                    level="MINOR",
                    response_sla_mins=20,
                    required_roles=["STEWARD", "SECURITY"],
                    notification_scope="ZONE",
                    escalation_policy="SUPERVISOR",
                ),
                EmergencySeverityMatrix(
                    level="MEDIUM",
                    response_sla_mins=15,
                    required_roles=["SECURITY", "MEDICAL"],
                    notification_scope="ZONE",
                    escalation_policy="COMMAND_POST",
                ),
                EmergencySeverityMatrix(
                    level="MAJOR",
                    response_sla_mins=10,
                    required_roles=["SECURITY", "MEDICAL", "OPERATIONS"],
                    notification_scope="VENUE",
                    escalation_policy="EXECUTIVE",
                ),
                EmergencySeverityMatrix(
                    level="HIGH",
                    response_sla_mins=8,
                    required_roles=["SECURITY", "MEDICAL", "POLICE"],
                    notification_scope="VENUE",
                    escalation_policy="EXECUTIVE",
                ),
                EmergencySeverityMatrix(
                    level="CRITICAL",
                    response_sla_mins=5,
                    required_roles=["SECURITY", "PARAMEDIC", "POLICE", "COMMAND"],
                    notification_scope="ALL_CHANNELS",
                    escalation_policy="DIRECTOR",
                ),
                EmergencySeverityMatrix(
                    level="CATASTROPHIC",
                    response_sla_mins=3,
                    required_roles=["ALL_RESPONDERS"],
                    notification_scope="EMERGENCY_BROADCAST",
                    escalation_policy="IMMEDIATE_EVAC",
                ),
            ]
        ...
        if not db.session.query(FirstResponder.query.exists()).scalar():
            responders = [
                ...
                FirstResponder(
                    name="Security Team Echo",
                    unit_type="SECURITY",
                    status="ON_SCENE",
                    current_zone_id=4,
                ),
                FirstResponder(
                    name="Fire Squad 1", unit_type="FIRE", status="AVAILABLE", current_zone_id=5
                ),
                FirstResponder(
                    name="Police K9 Unit",
                    unit_type="POLICE",
        ...
        if existing_incidents_count < 20:
            ...
            for i, ts in enumerate(timestamps):
                ...
                status = (
                    "RESOLVED"
                    if ts < datetime.now(timezone.utc) - timedelta(days=1)
                    else random.choice(["ACTIVE", "UNDER_INVESTIGATION", "RESOLVED"])
                )
            ...
            for ts in timestamps:
                events.append(
                    OperationEvent(
                        ...
                        payload={
                            "detail": "Routine stadium operational event logged.",
                            "shift": "Evening",
        ...
        if not db.session.query(CrowdZone.query.exists()).scalar():
            ...
            for cz in zones:
                for ts in timestamps:
                    ...
                    snapshots.append(CrowdSnapshot(zone_id=cz.id, occupancy=occ, timestamp=ts))
            ...
            for ts in timestamps:
                task = VolunteerTask(
                    description=random.choice(
                        [
                            "Wheelchair assistance at Gate A",
                            "Lost child escort",
                            "VIP entrance check",
                            "Medical tent standby",
                        ]
                    ),
        ...
        if existing_parking < 5:
            parking = [
                ParkingZone(
                    name="Lot A (VIP)",
                    max_capacity=500,
                    current_occupancy=random.randint(400, 500),
                    zone_type="VIP",
                ),
                ParkingZone(
                    name="Lot B (General)",
                    max_capacity=5000,
                    current_occupancy=random.randint(2000, 4800),
                    zone_type="General",
                ),
                ParkingZone(
                    name="Lot C (Accessible)",
                    max_capacity=300,
                    current_occupancy=random.randint(100, 250),
                    zone_type="ADA",
                ),
                ParkingZone(
                    name="Lot D (EV Charging)",
                    max_capacity=200,
                    current_occupancy=random.randint(50, 190),
                    zone_type="EV",
                ),
                ParkingZone(
                    name="Lot E (Media)", max_capacity=150, current_occupancy=140, zone_type="Media"
                ),
            ]
