...
def get_decisions():
    ...
    decisions = AIArbitrationDecision.query.order_by(AIArbitrationDecision.timestamp.desc()).limit(10).all()
    if not decisions:
        return jsonify([]), 200
...
def get_executive_dashboard():
    ...
    from models_volunteer import AccessibilityMetrics, Volunteer, AccessibilityRequest
    ...
    latest_acc_metrics = AccessibilityMetrics.query.order_by(AccessibilityMetrics.timestamp.desc()).first()
    ...
    crowd_density = (total_attendance / capacity_sum) if (capacity_sum and capacity_sum > 0) else None
    ...
    services_health = [
        {"service_name": "Operations AI", "status": "HEALTHY" if Incident.query.first() else "UNAVAILABLE"},
        {"service_name": "Crowd AI", "status": "HEALTHY" if StadiumZone.query.first() else "UNAVAILABLE"},
        {"service_name": "Volunteer AI", "status": "HEALTHY" if Volunteer.query.first() else "UNAVAILABLE"},
        {"service_name": "Transport AI", "status": "HEALTHY" if ShuttleRoute.query.first() else "UNAVAILABLE"},
...
def get_executive_kpis():
    ...
    incidents_count = db.session.execute(text("SELECT COUNT(*) FROM incidents WHERE status='OPEN'")).scalar() or 0
    satisfaction = max(1.0, 5.0 - (incidents_count * 0.1))
    
    # 4. Total Venue Revenue
    total_fans = db.session.execute(text("SELECT SUM(current_occupancy) FROM stadium_zones")).scalar() or 0
    revenue = total_fans * 42.50
    
    # 5. Security Posture
    active_responders = db.session.execute(
    ...
    avg_time = db.session.execute(
        text("SELECT COUNT(*) FROM emergency_incidents WHERE incident_type='MEDICAL' AND status='ACTIVE'")
    ).scalar() or 0
