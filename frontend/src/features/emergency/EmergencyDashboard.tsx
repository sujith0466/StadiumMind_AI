...
const Skeleton = () => (
  <div className="space-y-3" aria-label="Loading" aria-hidden="true">
...
const EmergencyDashboard: React.FC = () => {
  ...
  return (
    <PageContainer>
      ...
      {/* Header */}
      <header className="flex flex-wrap justify-between items-center border-b border-slate-800 pb-4 mb-6 gap-4">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <section className="bg-slate-900 rounded-xl border border-slate-800 p-5" aria-labelledby="escalate-incident-title">
            <h2 id="escalate-incident-title" className="text-lg font-bold text-red-300 mb-4">Escalate New Incident</h2>
            <div className="flex flex-wrap gap-3 items-end">
              <div>
                <label htmlFor="severity-select" className="block text-xs text-slate-400 mb-1">Severity</label>
                <select
                  id="severity-select"
                  value={newSeverity}
                  onChange={e => setNewSeverity(e.target.value)}
                  className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-red-500 focus:outline-none"
                >
              ...
              <div>
                <label htmlFor="zone-input" className="block text-xs text-slate-400 mb-1">Zone ID</label>
                <input
                  id="zone-input"
                  type="number" min={1} max={20} value={newZone}
                  onChange={e => setNewZone(Number(e.target.value))}
                  className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm w-24 focus:ring-2 focus:ring-red-500 focus:outline-none"
                />
              </div>
              <button
                onClick={handleEscalate}
                className="bg-red-700 hover:bg-red-800 text-white font-bold px-6 py-2 rounded-lg text-sm transition-colors focus:ring-2 focus:ring-red-500 focus:outline-none focus:ring-offset-2 focus:ring-offset-slate-900"
              >
                Escalate
              </button>
            </div>
          ...
          {/* Active incidents */}
          <section className="bg-slate-900 rounded-xl border border-slate-800 p-5" aria-labelledby="active-incidents-title">
            <h2 id="active-incidents-title" className="text-lg font-bold text-slate-200 mb-4">
              Active Incidents
        ...
        <section className="bg-slate-900 rounded-xl border border-slate-800 p-5" aria-labelledby="assistant-title">
          ...
          <div className="space-y-3">
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm placeholder-slate-600 resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500"
