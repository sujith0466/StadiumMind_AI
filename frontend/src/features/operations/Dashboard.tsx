  ...
  return (
    <PageContainer>
      ...
      <header className="flex justify-between items-center border-b border-slate-800 pb-4 mb-6">
        ...
        <span className="badge-info text-xs font-bold px-3 py-1 rounded-full text-blue-400 border-blue-400/30" aria-label="System status: Live">● LIVE</span>
      </header>

      {/* KPIs */}
      {loading ? (
        <section aria-label="Loading KPIs" className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Incident Table */}
        <section className="lg:col-span-2 bg-slate-900 rounded-xl border border-slate-800 p-5" aria-labelledby="incident-log-title">
                      ...
                      <td className="py-3">
                        {inc.status !== 'RESOLVED' && (
                          <button onClick={() => handleResolve(inc.id)}
                            ...
                            className="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1 rounded border border-slate-700 transition-colors focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            Resolve
                          </button>
                        )}
                      </td>
        ...
        {/* Action Panel */}
        <aside className="space-y-6" aria-label="Operations Actions">
          <section className="bg-slate-900 rounded-xl border border-slate-800 p-5">
            ...
            <div className="space-y-3">
              <div>
                ...
                <select id="severity-select" value={newSeverity} onChange={e => setNewSeverity(e.target.value)}
              ...
              <div>
                ...
                <input id="zone-input" type="number" min={1} value={newZone} onChange={e => setNewZone(Number(e.target.value))}
                  className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-blue-500 focus:outline-none" />
              </div>
              <button onClick={handleCreate}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-lg text-sm transition-colors mt-2 focus:ring-2 focus:ring-blue-500 focus:outline-none focus:ring-offset-2 focus:ring-offset-slate-900">
                Create & Request AI Triage
              </button>
            </div>
          </section>

          <section className="bg-slate-900 rounded-xl border border-slate-800 p-5 min-h-[150px]" aria-live="polite">
